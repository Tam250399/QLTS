...
{
    public class TaiSanDBSvcFactory : ITaiSanDBSvcFactory
    {
        #region Ctor
        ...
        #region Method
        #region TaiSan
        public TSDBModel PrepareTSDBModel(DBTaiSanModel model)
        {
            TSDBModel dbModel = new TSDBModel()
            {
                MA = model.QLDKTS_MA,
        ...
        public ResultTS GetAllTaiSans(int LoaiHinhTaiSanId = 0, int pageSize = int.MaxValue, int pageIndex = 0, int DonViId = 0)
        {
            ...
            ResultTS resultTaiSan = new ResultTS();
            List<TSDBModel> lst = new List<TSDBModel>();
            foreach (var itm in query)
            {
                ...
                dbModel.LST_BIEN_DONG = new List<BDDBModel>();
                if (lstbd != null)
                    foreach (var c in lstbd)
                    {
                        var x = c.ToModel<BDDBModel>();
        ...
        public MessageReturn UpdateTaiSan(TSDBModel dbModel, NguoiDung currentUser)
        {
            if (dbModel == null)
            {
        ...
        public MessageReturn UpdateListTaiSan(List<TSDBModel> ListModel, NguoiDung currentUser, decimal nguontaisan)
        {
            int TotalErr = 0;
            int TotalSuc = 0;
            int TotalExist = 0;
            ...
            List<TSDBModel> LstErr = new List<TSDBModel>();
            bool IsVaild = true;
            ...
            foreach (var dbModel in ListModel)
            {
                ...
                #endregion
                #region validate Biến động
                List<BDDBModel> ListBienDong = dbModel.LST_BIEN_DONG.OrderBy(m => m.NGAY_BIEN_DONG.toDateSys()).OrderBy(m => m.THU_TU_BIEN_DONG_ID).ToList();
                foreach (var item in ListBienDong)
                {
                    result = ValidateThongTinBienDong(item);
                }
                ...
                // nguyên giá ban đầu
                BDDBModel bienDongFirst = ListBienDong.Where(m => m.LOAI_BIEN_DONG_ID == (decimal)enumLOAI_LY_DO_TANG_GIAM.TANG_TOAN_BO || m.LOAI_BIEN_DONG_ID == (decimal)enumLOAI_LY_DO_TANG_GIAM.NHAP_SO_DU_DAU_KY).FirstOrDefault();
                ...
                {
                    foreach (HaoMonTaiSanDBModel hm in dbModel.LST_HAO_MON)
                    {
                        var rs = _validateFactory.CheckHaoMonTaiSan(hm);
                ...
                {
                    foreach (KhauHaoTaiSanDBModel kh in dbModel.LST_KHAU_HAO)
                    {
                        var rs = _validateFactory.CheckKhauHaoTaiSan(kh);
            ...
            if (TotalErr > 0)
            {
                foreach (TSDBModel item in LstErr)
                {
            ...
            {
                foreach (TSDBModel item in ListModel)
                {
                    DBTaiSan _dBTaiSan = _dbTaiSanService.GetTaiSanByMa(DB_MA: item.DB_MA, nguonTaiSanId: nguontaisan);
                    if (_dBTaiSan != null)
                    {
        ...
        public MessageReturn UpdateBienDong(List<BDDBModel> ListBienDong, NguoiDung currentUser, decimal nguontaisan)
        {
        ...
        public MessageReturn PrepareBienDong(BienDong bienDong, BDDBModel model)
        {
            if (bienDong == null)
            {
        ...
        public MessageReturn PrepareBienDongChiTiet(BienDongChiTiet bienDongChiTiet, BDDBModel model)
        {
        ...
        public void UpDateNguonVon(BDDBModel model)
        {
        ...
        public MessageReturn ValidateThongTinTaiSan(TSDBModel dbModel)
        {
            ...
            else
            {
                foreach (var item in dbModel.LST_BIEN_DONG)
                {
                    ...
                    {
                        item.LOAI_TAI_SAN_MA = dbModel.LOAI_TAI_SAN_MA;
                    }
                }
            }

            if (dbModel.LOAI_TAI_SAN_MA != null)
            {
                loaiTaiSan = _loaiTaiSanService.GetLoaiTaiSanByMa(dbModel.LOAI_TAI_SAN_MA);
                dbModel.LOAI_TAI_SAN_ID = loaiTaiSan.ID;
                LoaiHinhTaiSanId = loaiTaiSan.LOAI_HINH_TAI_SAN_ID.Value;
                dbModel.LOAI_HINH_TAI_SAN_ID = LoaiHinhTaiSanId;
            }
            else dbModel.LOAI_TAI_SAN_MA = null;
            if (dbModel.LOAI_TAI_SAN_DON_VI_MA != null)
            {
                LoaiTaiSanDonVi loaiTaiSanDonVi = _loaiTaiSanDonViService.GetLoaiTaiSanVoHinhByMa(dbModel.LOAI_TAI_SAN_DON_VI_MA);
                dbModel.LOAI_TAI_SAN_DON_VI_ID = loaiTaiSanDonVi.ID;
                LoaiHinhTaiSanId = loaiTaiSanDonVi.LOAI_HINH_TAI_SAN_ID.Value;
                dbModel.LOAI_HINH_TAI_SAN_ID = LoaiHinhTaiSanId;
            }
            else dbModel.LOAI_TAI_SAN_DON_VI_MA = null;
            ...
            // đơn vị bộ phận
            if (dbModel.DON_VI_BO_PHAN_MA != null)
            {
                DonViBoPhan donViBoPhan = _donViBoPhanService.GetDonViBoPhanByMa(dbModel.DON_VI_BO_PHAN_MA);
                if (donViBoPhan == null)
                {
                    return MessageReturn.CreateNotFoundMessage("DON_VI_BO_PHAN_MA not exist");
                }
                else dbModel.DON_VI_BO_PHAN_ID = donViBoPhan.ID;
            }
            #endregion
        ...
        public MessageReturn ValidateThongTinBienDong(BDDBModel model)
        {
            ...
            if (model.LOAI_TAI_SAN_MA != null)
            {
                //decimal LoaiTaiSanId = model.LOAI_TAI_SAN_ID.Value;
                var loaiTaiSan = _loaiTaiSanService.GetLoaiTaiSanByMa(model.LOAI_TAI_SAN_MA);
                if (loaiTaiSan == null)
                {
                    return MessageReturn.CreateNotFoundMessage($"BIEN_DONG:ID_DB= { model.ID_DB}-LOAI_TAI_SAN_MA not exist");
                }
                else
                {
                    ...
                    LoaiHinhTaiSanId = loaiTaiSan.LOAI_HINH_TAI_SAN_ID.Value;
                    model.LOAI_TAI_SAN_ID = loaiTaiSan.ID;
                }
            }
            else model.LOAI_TAI_SAN_MA = null;
            if (model.LOAI_TAI_SAN_DON_VI_MA != null)
            {
                var loaiTaiSanDonVi = _loaiTaiSanDonViService.GetLoaiTaiSanVoHinhByMa(model.LOAI_TAI_SAN_DON_VI_MA);
                LoaiHinhTaiSanId = loaiTaiSanDonVi.LOAI_HINH_TAI_SAN_ID.Value;
            }
            else model.LOAI_TAI_SAN_DON_VI_MA = null;
            ...
            if (model.LY_DO_BIEN_DONG_MA == null)
            {
                return MessageReturn.CreateNotFoundMessage($"BIEN_DONG:ID_DB= { model.ID_DB}-LY_DO_BIEN_DONG_MA null");
            }
            else
            {
                //decimal LyDoBienDongId = model.LY_DO_BIEN_DONG_ID.Value;
                LyDoBienDong lyDoBienDong = _lyDoBienDongService.GetLyDoBienDongByMa(model.LY_DO_BIEN_DONG_MA);
                if (lyDoBienDong == null)
                {
                    return MessageReturn.CreateErrorMessage($"BIEN_DONG:ID_DB= { model.ID_DB}-LY_DO_BIEN_DONG_MA not exist");
                }
                model.LY_DO_BIEN_DONG_ID = lyDoBienDong.ID;
        ...
        public MessageReturn ValidateNguonVon(BDDBModel model, decimal LoaiHinhTaiSanId)
        {
        ...
        #endregion
        public TaiSan InsertTaiSanTemp(TSDBModel dbModel, NguoiDung currentUser, decimal nguontaisan)
        {
            ...
            if (dbModel.LOAI_TAI_SAN_MA != null)
            {
                LoaiTaiSan loaiTaiSan = _loaiTaiSanService.GetLoaiTaiSanByMa(dbModel.LOAI_TAI_SAN_MA);
                MaLoaiTaiSan = loaiTaiSan != null ? loaiTaiSan.MA : null;
            }
            else if (dbModel.LOAI_TAI_SAN_DON_VI_MA != null)
            {
                LoaiTaiSanDonVi loaiTaiSanDonVi = _loaiTaiSanDonViService.GetLoaiTaiSanVoHinhByMa(dbModel.LOAI_TAI_SAN_DON_VI_MA);
            ...
            _hoatDongService.InsertHoatDong(currentUser, enumHoatDong.TaoMoi, "Thêm mới dữ liệu đồng bộ t", 0, "DB_TAI_SAN", dBTaiSan);
            return taiSan;
        }
