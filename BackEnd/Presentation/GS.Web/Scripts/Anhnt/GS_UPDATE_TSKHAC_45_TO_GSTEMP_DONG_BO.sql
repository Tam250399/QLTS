CREATE OR REPLACE FUNCTION GS_UPDATE_TSKHAC_45_TO_GSTEMP_DONG_BO (
...
-- 115	Qu?n lý nhà n??c 	  2	              	      1	          1
-- 116	H?SN-Không KD	      2	              1	      1	          2
-- 117	H?SN-Kinh doanh	    2	              1	      1	          3
-- 118	S? d?ng khác	      2	              0	      1	          6
-- 119	H?SN-Cho thuê	      2	              0	      1	          5
-- 120	H?SN-LDLK	          2	              	      1	          4
-- 190	Qu?n lý d? án	      2	              2	      1	          1
-- 191	S? d?ng khác	      2	              2	      1	          2

-- 152	Qu?n lý nhà n??c	  2	              	      1	          1
-- 153	H?SN-Không KD	      2	              1	      1	          2
-- 154	H?SN-Kinh doanh	    2	              1	      1	          3
-- 155	S? d?ng khác	      2	              0	      1	          6
-- 156	H?SN-LDLK	          2	              	      1	          4
-- 157	H?SN-Cho thuê	      2	              0	      1	          5
-- 194	Qu?n lý d? án	      2	              2	      1	          1
-- 195	S? d?ng khác	      2	              2	      1	          2

-- 158	Qu?n lý nhà n??c	  2	              	      1	          1
-- 159	H?SN-Không KD	      2	              1	      1	          2
-- 160	H?SN-Kinh doanh	    2	              1	      1	          3
-- 161	S? d?ng khác	      2	              0	      1	          6
-- 162	H?SN-LDLK	          2	              	      1	          4
-- 163	H?SN-Cho thuê	      2	              0	      1	          5
-- 196	Qu?n lý d? án	      2	              2	      1	          1
-- 197	S? d?ng khác	      2	              2	      1	          2

-- 164	Qu?n lý nhà n??c	  2	              	      1	
-- 165	H?SN-Không KD	      2	              1	      1	
-- 166	H?SN-Kinh doanh	    2	              1	      1	
-- 167	S? d?ng khác	      2	              0	      1	
-- 168	H?SN-LDLK	          2	              	      1	
-- 169	H?SN-Cho thuê	      2	              0	      1	
-- 198	Qu?n lý d? án	      2	              2	      1	
-- 199	S? d?ng khác	      2	              2	      1	
    SELECT
        *
    INTO GSTEMPDONGBO
    FROM
...
--
    ...
    FOR R_BD_KHAC IN (
        ...
        BIENDONGGUID := RAW_TO_GUID(LPAD(R_BD_KHAC.BD_KHAC_ID, 32, '0'));
        IF R_BD_KHAC.LOAI_BIEN_DONG = 4 THEN
            R_BD_KHAC.MA_LY_DO := '047';
        END IF;
        BEGIN
            SELECT
                GSTG.*
            ...
            WHERE
                ROWNUM = 1
                AND GSTG.MA_OLD = R_BD_KHAC.MA_LY_DO;
            ...
            WHERE
                MA_OLD = (
                    ...
                    FROM
                        DM_LOAITAISAN
                    WHERE
                        LOAI_TAI_SAN_ID = R_BD_KHAC.LOAI_TAI_SAN_ID
                )
        ...
        IF R_BD_KHAC.MA_LY_DO IN (
            ...
            BEGIN
                ...
                WHERE
                    ...
                    AND NGAY_BIEN_DONG < R_BD_KHAC.NGAY_BIEN_DONG
                ORDER BY
                    NGAY_BIEN_DONG DESC
        ...
        "NGUYEN_GIA":'
                        || RTRIM(TO_CHAR((CASE
            WHEN R_BD_KHAC.MA_LY_DO IN(
                '050', '051', '052'
            ) THEN
                ABS(NVL(R_BD_KHAC.NGUYEN_GIA_NS, 0) + NVL(R_BD_KHAC.NGUYEN_GIA_KHAC, 0) - NVL(BDKHACCUOI.NGUYEN_GIA_NS, 0) - NVL(BDKHACCUOI.NGUYEN_GIA_KHAC, 0))
            ELSE NVL(R_BD_KHAC.NGUYEN_GIA_NS, 0) + NVL(R_BD_KHAC.NGUYEN_GIA_KHAC, 0)
        END), 'FM9999999999999999999999990.99'), '.')
                        || ',
        "NGAY_BIEN_DONG":"'
                        || TO_CHAR(R_BD_KHAC.NGAY_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "DON_VI_NHAN_DIEU_CHUYEN_MA":"'
                        || R_BD_KHAC.MA_DONVI_NHAN_DIEU_CHUYEN
                        || '",
        "LOAI_BIEN_DONG_ID":'
                        || R_BD_KHAC.LOAI_BIEN_DONG
                        || ', 
        "LOAI_TAI_SAN_MA":"'
                        || GSMAPLTS.MA_NEW
                        || '",
        ...
        "HINH_THUC_XU_LY_ID":'
                        || (
            CASE
                WHEN NVL(R_BD_KHAC.HINH_THUC_THANH_LY, R_BD_KHAC.HINH_THUC_BAN) = '001' THEN
                    2
                WHEN NVL(R_BD_KHAC.HINH_THUC_THANH_LY, R_BD_KHAC.HINH_THUC_BAN) = '002' THEN
                    3
                WHEN NVL(R_BD_KHAC.HINH_THUC_THANH_LY, R_BD_KHAC.HINH_THUC_BAN) = '003' THEN
                    1
            END
        )
                        || ',
        "DON_VI_MA":"'
                        ...
                        || '",
        "NGAY_SU_DUNG":"'
                        || TO_CHAR(R_BD_KHAC.THOI_GIAN_SU_DUNG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "TRANG_THAI":'
                        || NVL(R_BD_KHAC.DUYET_BIEN_DONG, 1)
                        || ',
        "THONG_SO_KY_THUAT":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_KHAC.THONG_SO_KT)
                        || '",
        "GHI_CHU":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_KHAC.MO_TA_CHUNG)
                        || '",
        "NGAY_DUYET":"'
                        || TO_CHAR(R_BD_KHAC.NGAY_DUYET_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "GIA_TRI_CON_LAI":'
                        || RTRIM(TO_CHAR(R_BD_KHAC.GIA_TRI_CON_LAI, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "KH_GIA_TINH_KHAU_HAO":'
                        || RTRIM(TO_CHAR(NVL(R_BD_KHAC.KH_NGUYEN_GIA_NS, 0) + NVL(R_BD_KHAC.KH_NGUYEN_GIA_KHAC, 0) + NVL(R_BD_KHAC.KH_NGUYEN_GIA_ODA, 0) + NVL(R_BD_KHAC.KH_NGUYEN_GIA_VIEN_TRO, 0), 'FM9999999999999999999999990.99'
                        ), '.')
                        || ',
        "KH_GIA_TRI_CON_LAI":'
                        || RTRIM(TO_CHAR(R_BD_KHAC.KH_GIA_TRI_CON_LAI, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "KH_THANG_CON_LAI":'
                        || R_BD_KHAC.SO_THANG_CON_SU_DUNG
                        || ',
        "QUYET_DINH_SO":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_KHAC.SO_QD)
                        || '",
        "QUYET_DINH_NGAY":"'
                        || TO_CHAR(R_BD_KHAC.NGAY_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "PHI_THU":'
                        || RTRIM(TO_CHAR(R_BD_KHAC.SO_TIEN_THU_DUOC, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "HS_KHAC":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_KHAC.SO_QD)
                        || '"},';
        ...
        NGUONVONJSON := NGUONVONJSON
                        ...
                        || R_BD_KHAC.NGUYEN_GIA_NS
                        || ', "BIEN_DONG_GUID":"'
                        || BIENDONGGUID
                        || '"},
        {"NGUON_VON_ID":3, "GIA_TRI":'
                        || R_BD_KHAC.NGUYEN_GIA_KHAC
                        || ', "BIEN_DONG_GUID":"'
                        || BIENDONGGUID
                        || '"},';
        ...
        IF GSTEMPDONGBO.MA_DU_AN IS NULL THEN
            HIENTRANGJSON := HIENTRANGJSON
                             ...
                             || (
                CASE
                    WHEN PLOAI_HINH_TAI_SAN = 6 THEN
                        '115'
                    WHEN PLOAI_HINH_TAI_SAN = 8 THEN
                        '152'
                    WHEN PLOAI_HINH_TAI_SAN = 9 THEN
                        '158'
                    WHEN PLOAI_HINH_TAI_SAN = 10 THEN
                        '164'
                END
            )
                             || ',
            "TEN_HIEN_TRANG":"Qu?n lý nhà n??c",
            "KIEU_DU_LIEU_ID":2,
            "NHOM_HIEN_TRANG_ID":null,
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_KHAC.HIEN_TRANG_SU_DUNG IN (
                        '1',
                        '0'
                    ) OR R_BD_KHAC.HIEN_TRANG_SU_DUNG IS NULL THEN
                        'true'
                    ELSE 'false'
                END
            )
            ...
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_KHAC.HIEN_TRANG_SU_DUNG = '3' THEN
                        'true'
                    ELSE 'false'
                END
            )
            ...
            {"HIEN_TRANG_ID":'
                ...
                END
                             ...
                             || ',
            "TEN_HIEN_TRANG":"H?SN-Kinh doanh",
            "KIEU_DU_LIEU_ID":2,
            "NHOM_HIEN_TRANG_ID":1,
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_KHAC.HIEN_TRANG_SU_DUNG = '2' THEN
                        'true'
                    ELSE 'false'
                END
            )
            ...
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_KHAC.HIEN_TRANG_SU_DUNG = '4' THEN
                        'true'
                    ELSE 'false'
                END
            )
    ...
    BEGIN
        SELECT
            TO_CLOB('{"THONG_SO_KY_THUAT":"'
                    || GS_STRING_TO_STRINGJSON(NVL(BDKHACCUOI.THONG_SO_KT, TS.THONG_SO_KT))
                    || '","THONG_SO_KY_HIEU":"'
...
--            DBMS_OUTPUT.PUT_LINE(PMA_TAI_SAN);
    ...
    HAOMONJSON := REPLACE('['
                          || HAOMONJSON
                          || ']', '":,', '":null,');
--    DBMS_OUTPUT.PUT_LINE(haomonJson);
    ...
    COMMIT;
    RETURN 0;
END GS_UPDATE_TSKHAC_45_TO_GSTEMP_DONG_BO;