...
namespace GS.WebApi.Validator.TaiSan
{
    ...
    public class KhauHaoValidator : BaseGSValidator<KhauHaoDBModel> 
    {
        public KhauHaoValidator(IValidateFactory validateFactory)
        {
            RuleFor(m => m).Custom((model, context) =>
            {
                var result = validateFactory.CheckChiTietKhauHaoTaiSan(model,false);
                if (result.Count > 0)
                {
