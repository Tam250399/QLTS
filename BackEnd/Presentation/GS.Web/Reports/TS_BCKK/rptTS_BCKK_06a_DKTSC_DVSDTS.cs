...
namespace GS.Web.Reports.TS_BCKK
{
    public partial class rptTS_BCKK_06a_DKTSC_DVSDTS
    {
		...
		{
			var idCapDonVi =  GetCurrentColumnValue<decimal?>("KHOI_DON_VI_CU");
			if (idCapDonVi!= null)
			{
				(sender as XRTableCell).Text = idCapDonVi.GetValueOrDefault(100).ToStringCapDonVi();
			}
		}

		private void tableCell34_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
		{
			var idCapDonVi = GetCurrentColumnValue<decimal?>("KHOI_DON_VI_MOI");
			if (idCapDonVi != null)
			{
				(sender as XRTableCell).Text = idCapDonVi.GetValueOrDefault(100).ToStringCapDonVi();
			}
