...
namespace GS.Core.Infrastructure
{
    ...
    /// </summary>
    public class GSFileProvider : PhysicalFileProvider, IGSFileProvider
    {
        /// <summary>
        /// Initializes a new instance of a GSFileProvider
        /// </summary>
        /// <param name="hostingEnvironment">Hosting environment</param>
        public GSFileProvider(IHostingEnvironment hostingEnvironment)
        ...
        public virtual void DeleteFile(string filePath, bool isKeepOnDisk = false)
        {
            if (!FileExists(filePath))
                return;
            if (isKeepOnDisk)
            {
                var _filedel = GetFileName(filePath);
                File.Move(filePath, Combine(MapPath(GSDataSettingsDefaults.FolderTrashFiles), _filedel));
            }
            else
        ...
        public virtual byte[] GetWorkFileContentOnDisk(FileCongViec item)
        {
            var _fileStore = Combine(MapPath(GSDataSettingsDefaults.FolderWorkFiles), item.NGAY_TAO.ToPathFolderStore(), item.GUID.ToString() + item.DUOI_FILE);
            return ReadAllBytes(_fileStore);
        }
    }
}