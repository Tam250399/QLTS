...
{
    public class TaiSanValidator : BaseGSValidator<TaiSanModel>
    {
        public TaiSanValidator(
            ...
            )
        {
            ...
            RuleFor(x => x.LY_DO_TANG_ID).NotEqual(0, null).WithMessage("Chưa chọn lý do tăng");
            ...
            RuleFor(x => x.NGUYEN_GIA).Must((model, NGUYEN_GIA) =>
            {
            ...
            RuleFor(x => x.GIA_TRI_SU_DUNG_DAT.GIA_TRI_QUYEN_SD_DAT).Must((model, Gia_QSD) =>
            {
            ...
            RuleFor(x => x.NGUYEN_GIA).Must((model, Gia_QSD) =>
            {
                if (model.LOAI_HINH_TAI_SAN_ID == (decimal)enumLOAI_HINH_TAI_SAN.DAT)
                {
                    if (Gia_QSD > model.NGUYEN_GIA)
                    {
                        return false;
                    }
                    return true;
