...
namespace GS.NewAPI.Factories
{
    public class BienDongChiTietModelFactory : IBienDongChiTietModelFactory
    {
        ...
        public BienDongChiTiet UpdateBienDongChiTiet(TaiSanModel item, BienDongChiTiet bdct, BienDong bienDong)
        {
            if (item != null)
            {
                //test
                ...
                {
                    case (int)enumLOAI_HINH_TAI_SAN.DAT:
                        bdct.DIA_CHI = item.DIA_CHI;
                        bdct.HS_CNQSD_SO = item.HO_SO_GIAY_TO.CHUNG_NHAN_QUYEN_SD_DAT;
                        bdct.HS_CNQSD_NGAY = item.HO_SO_GIAY_TO.NGAY_CAP.CHUNG_NHAN_QUYEN_SD_DAT;
                        bdct.HS_QUYET_DINH_GIAO_SO = item.HO_SO_GIAY_TO.QD_GIAO_DAT;
                        bdct.HS_QUYET_DINH_GIAO_NGAY = item.HO_SO_GIAY_TO.NGAY_CAP.QD_GIAO_DAT;
                        bdct.HS_QUYET_DINH_CHO_THUE_SO = item.HO_SO_GIAY_TO.QD_CHO_THUE_DAT;
                        bdct.HS_QUYET_DINH_CHO_THUE_NGAY = item.HO_SO_GIAY_TO.NGAY_CAP.QD_CHO_THUE_DAT;
                        bdct.HS_HOP_DONG_CHO_THUE_SO = item.HO_SO_GIAY_TO.HOP_DONG_CHO_THUE_DAT;
                        bdct.HS_HOP_DONG_CHO_THUE_NGAY = item.HO_SO_GIAY_TO.NGAY_CAP.HOP_DONG_CHO_THUE_DAT;
                        bdct.HS_KHAC = item.HO_SO_GIAY_TO.GIAY_TO_KHAC;
                        break;
                    case (int)enumLOAI_HINH_TAI_SAN.NHA:
                        // dữ liệu kiểu nhà
                        //model.NHA_SO_TANG = item.SO_TANG;
            ...
            //yeuCau_json.DATA_JSON = null;
            //bdct.DATA_JSON = yeuCau_json.toStringJson();
            var lstHienTrang = _hienTrangService.GetHienTrangs(LoaiHinhTsId: bienDong.LOAI_HINH_TAI_SAN_ID, isTSDA: _donViService.isDonViBanQuanLyDuAn(bienDong.DON_VI_ID.GetValueOrDefault()));
            var lstObjHienTrang = new List<ObjHienTrang>();
            if (bienDong.LOAI_HINH_TAI_SAN_ID == (decimal)enumLOAI_HINH_TAI_SAN.DAT)
            {
                foreach (var ht in lstHienTrang)
                {
                    var obj = new ObjHienTrang();
            ...
            {   if(bienDong.LOAI_HINH_TAI_SAN_ID == (decimal)enumLOAI_HINH_TAI_SAN.NHA)
                {
                    foreach (var ht in lstHienTrang)
                    {
                        var obj = new ObjHienTrang();
        ...
        public BienDongChiTietModel getBienDongChiTiet (decimal bienDongId)
        {
            var bdct = _bienDongChiTietService.GetBienDongChiTietByBDId (bienDongId);
            return bdct.ToModel<BienDongChiTietModel>();
        }
    }
}
