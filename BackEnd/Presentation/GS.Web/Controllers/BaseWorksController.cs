...
namespace GS.Web.Controllers
{
    ...
    public class BaseWorksController : BaseController
    {
        ...
        protected IActionResult ShowViewReport(DevExpress.XtraReports.UI.XtraReport model)
        {
            return View("ViewReport", model);
        }
        protected void UpdateSessionSearchModel<T>(bool isLoadSession) where T:BaseSearchModel
        {
            var SearchModel = HttpContext.Session.GetObject<T>(enumTENCAUHINH.KeySearch + typeof(T).Name);
            if(SearchModel !=null)
            {
                SearchModel.IsLoadSession = isLoadSession;
                HttpContext.Session.SetObject(enumTENCAUHINH.KeySearch + typeof(T).Name, SearchModel);
            }
