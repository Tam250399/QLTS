CREATE OR REPLACE PROCEDURE DELETE_ALL_TAI_SAN AS 
BEGIN
 FOR r_product IN (
        SELECT * FROM TS_TAI_SAN where MA_DB is not null
    ) 
  LOOP
    DELETE ts_tai_san_nguon_von where tai_san_id = r_product.ID
    or BIEN_DONG_ID in (select id from bd_bien_dong where tai_san_id = r_product.ID);
