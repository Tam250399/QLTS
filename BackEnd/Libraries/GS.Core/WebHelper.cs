    ...
    public partial class WebHelper : IWebHelper
    {
        ...
        public virtual string GetCurrentIpAddress()
        {
            ...
            try
            {
                ...
                if (_httpContextAccessor.HttpContext.Request.Headers != null)
                {
                    ...
                    var forwardedHttpHeaderKey = GSHttpDefaults.XForwardedForHeader;
        ...
        public virtual bool IsCurrentConnectionSecured()
        {
            ...
            if (_hostingConfig.UseHttpClusterHttps)
                return _httpContextAccessor.HttpContext.Request.Headers[GSHttpDefaults.HttpClusterHttpsHeader].ToString().Equals("on", StringComparison.OrdinalIgnoreCase);

            //use HTTP_X_FORWARDED_PROTO?
            if (_hostingConfig.UseHttpXForwardedProto)
                return _httpContextAccessor.HttpContext.Request.Headers[GSHttpDefaults.HttpXForwardedProtoHeader].ToString().Equals("https", StringComparison.OrdinalIgnoreCase);
        ...
        public virtual void RestartAppDomain(bool makeRedirect = false)
        {
            ...
            if (!success)
            {
                throw new GSException("nopCommerce needs to be restarted due to a configuration change, but was unable to do so." + Environment.NewLine +
        ...
        public virtual bool IsPostBeingDone
        {
            get
            {
                if (_httpContextAccessor.HttpContext.Items[GSHttpDefaults.IsPostBeingDoneRequestItem] == null)
                    return false;

                return Convert.ToBoolean(_httpContextAccessor.HttpContext.Items[GSHttpDefaults.IsPostBeingDoneRequestItem]);
            }

            set => _httpContextAccessor.HttpContext.Items[GSHttpDefaults.IsPostBeingDoneRequestItem] = value;
        }
