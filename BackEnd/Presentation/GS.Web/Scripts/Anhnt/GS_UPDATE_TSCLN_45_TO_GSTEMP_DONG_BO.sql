create or replace FUNCTION "GS_UPDATE_TSCLN_45_TO_GSTEMP_DONG_BO" (
    ...
    TRANGTHAI              NUMBER;
    BDDAU                  BD_KHAC%ROWTYPE;
    BDTAISAN               BD_TAI_SAN%ROWTYPE;
...
-- 172	Qu?n lý nhà n??c	  2	                      1	          1
-- 173	H?SN-Không KD	      2	              1	      1	          2
-- 174	H?SN-Kinh doanh	    2	              1	      1	          3
-- 175	S? d?ng khác	      2	              0	      1	          6
-- 176	H?SN-LDLK	          2	              0	      1	          4
-- 177	H?SN-Cho thuê	      2	              0	      1	          5
-- 192	Qu?n lý d? án	      2	              2	      1	          1
-- 193	S? d?ng khác	      2	              2	      1	          2
    SELECT
        *
    INTO GSTEMPDONGBO
    FROM
    ...
    FOR R_BD_CLN IN (
        ...
        BIENDONGGUID := RAW_TO_GUID(LPAD(R_BD_CLN.BD_KHAC_ID, 32, '0'));
        IF R_BD_CLN.LOAI_BIEN_DONG = 4 THEN
            R_BD_CLN.MA_LY_DO := '047';
        END IF;
        BEGIN
            SELECT
                GSTG.*
            ...
            WHERE
                ROWNUM = 1
                AND GSTG.MA_OLD = R_BD_CLN.MA_LY_DO;
            ...
            WHERE
                MA_OLD = (
                    ...
                    FROM
                        DM_LOAITAISAN
                    WHERE
                        LOAI_TAI_SAN_ID = R_BD_CLN.LOAI_TAI_SAN_ID
                )
        ...
        FROM
            BD_TAI_SAN
        WHERE
            MA_TAI_SAN = R_BD_CLN.MA_TAI_SAN
            AND BD_ID = R_BD_CLN.BD_KHAC_ID;
        ...
        BIENDONGJSON := BIENDONGJSON
                        ...
                        || '",
        "NGUYEN_GIA":'
                        || RTRIM(TO_CHAR(NVL(BDTAISAN.NGUYEN_GIA_NS, 0) + NVL(BDTAISAN.NGUYEN_GIA_KHAC, 0) + NVL(BDTAISAN.NGUYEN_GIA_ODA, 0) + NVL(BDTAISAN.NGUYEN_GIA_VIEN_TRO, 0), 'FM9999999999999999999999990.99'
                        ), '.')
                        || ',
        "NGAY_BIEN_DONG":"'
                        || TO_CHAR(R_BD_CLN.NGAY_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "DON_VI_NHAN_DIEU_CHUYEN_MA":"'
                        || R_BD_CLN.MA_DONVI_NHAN_DIEU_CHUYEN
                        || '",
        "LOAI_BIEN_DONG_ID":'
                        || R_BD_CLN.LOAI_BIEN_DONG
                        || ',
        "LOAI_TAI_SAN_MA":"'
                        || GSMAPLTS.MA_NEW
                        || '",
        ...
        "HINH_THUC_XU_LY_ID":'
                        || (
            CASE
                WHEN R_BD_CLN.HINH_THUC_THANH_LY = '001' OR R_BD_CLN.HINH_THUC_BAN = '001' THEN
                    2
                WHEN R_BD_CLN.HINH_THUC_THANH_LY = '002' OR R_BD_CLN.HINH_THUC_BAN = '002' THEN
                    3
                WHEN R_BD_CLN.HINH_THUC_THANH_LY = '003' OR R_BD_CLN.HINH_THUC_BAN = '003' THEN
                    1
            END
        )
                        || ',
        "IS_BAN_THANH_LY":'
                        || (
            CASE
                WHEN R_BD_CLN.HINH_THUC_THANH_LY IN (
                    '001',
                    '002',
                    '003'
                ) OR R_BD_CLN.HINH_THUC_BAN IN (
                    '001',
                    '002',
                    '003'
                ) THEN
        ...
        "LY_DO_BIEN_DONG_TEN":"'
                        ...
                        || '",
        "THONG_SO_KY_THUAT":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_CLN.THONG_SO_KT)
                        || '",
        "GHI_CHU":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_CLN.MO_TA_CHUNG)
                        || '",
        "DON_VI_MA":"'
                        || R_BD_CLN.MA_DON_VI
                        || '",
        "NGAY_SU_DUNG":"'
                        || TO_CHAR(R_BD_CLN.THOI_GIAN_SU_DUNG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "THONG_SO_KY_THUAT":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_CLN.THONG_SO_KT)
                        || '",
        "TRANG_THAI":'
                        || NVL(R_BD_CLN.DUYET_BIEN_DONG, 1)
                        || ',
        "NGAY_DUYET":"'
                        || TO_CHAR(R_BD_CLN.NGAY_DUYET_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "GIA_TRI_CON_LAI":'
                        || RTRIM(TO_CHAR(R_BD_CLN.GIA_TRI_CON_LAI, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "KH_GIA_TINH_KHAU_HAO":'
                        || RTRIM(TO_CHAR(NVL(R_BD_CLN.KH_NGUYEN_GIA_NS, 0) + NVL(R_BD_CLN.KH_NGUYEN_GIA_KHAC, 0) + NVL(R_BD_CLN.KH_NGUYEN_GIA_ODA, 0) + NVL(R_BD_CLN.KH_NGUYEN_GIA_VIEN_TRO, 0), 'FM9999999999999999999999990.99'
                        ), '.')
                        || ',
        "KH_GIA_TRI_CON_LAI":'
                        || RTRIM(TO_CHAR(R_BD_CLN.KH_GIA_TRI_CON_LAI, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "KH_THANG_CON_LAI":'
                        || R_BD_CLN.SO_THANG_CON_SU_DUNG
                        || ',
        "QUYET_DINH_SO":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_CLN.SO_QD)
                        || '",
        "QUYET_DINH_NGAY":"'
                        || TO_CHAR(R_BD_CLN.NGAY_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "PHI_THU":'
                        || RTRIM(TO_CHAR(R_BD_CLN.SO_TIEN_THU_DUOC, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "HS_KHAC":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_CLN.SO_QD)
                        || '"},';
        ...
        NGUONVONJSON := NGUONVONJSON
                        ...
                        || R_BD_CLN.NGUYEN_GIA_NS
                        || ', "BIEN_DONG_GUID":"'
                        || BIENDONGGUID
                        || '"},{"NGUON_VON_ID":3, "GIA_TRI":'
                        || R_BD_CLN.NGUYEN_GIA_KHAC
                        || ', "BIEN_DONG_GUID":"'
                        || BIENDONGGUID
                        || '"},';
        ...
        IF GSTEMPDONGBO.MA_DU_AN IS NULL THEN
            ...
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_CLN.HIEN_TRANG_SU_DUNG IN (
                        '1',
                        '0'
                    ) OR R_BD_CLN.HIEN_TRANG_SU_DUNG IS NULL THEN
                        'true'
                    ELSE 'false'
                END
            )
            ...
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_CLN.HIEN_TRANG_SU_DUNG = '3' THEN
                        'true'
                    ELSE 'false'
                END
            )
            ...
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_CLN.HIEN_TRANG_SU_DUNG = '2' THEN
                        'true'
                    ELSE 'false'
                END
            )
            ...
            "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_CLN.HIEN_TRANG_SU_DUNG = '4' THEN
                        'true'
                    ELSE 'false'
                END
            )
...
--    DBMS_OUTPUT.PUT_LINE(haomonJson);
    ...
    BEGIN
        SELECT
            TO_CLOB('{"NAM_SINH":'
                    ||(
                CASE
                    WHEN NVL(BDDAU.NAM_SX, TS.NAM_SX) IS NULL THEN
                        'null'
                    ELSE
                        NVL(BDDAU.NAM_SX, TS.NAM_SX)
                        || ''
                END
            )
                    || '}')
            ...
            (
                CASE
                    WHEN LOAI_BIEN_DONG = 5
                        ...
                        4
                    WHEN LOAI_BIEN_DONG = 1
                         AND DUYET_BIEN_DONG = 3 THEN
                        3
                    ELSE
    ...
    SET
        ...
        GSTEMP.NUOC_SAN_XUAT_MA = (
            CASE
                WHEN NVL(BDDAU.MA_QUOC_GIA, TSKHAC.MA_QUOC_GIA) = '999' THEN
                    'VN'
                WHEN NVL(BDDAU.MA_QUOC_GIA, TSKHAC.MA_QUOC_GIA) NOT IN (
                    SELECT
                        MA_QUOC_GIA
                    FROM
                        DM_QUOCGIA
                    ...
                    'VN'
                ELSE
                    NVL(BDDAU.MA_QUOC_GIA, TSKHAC.MA_QUOC_GIA)
            END
        ),
        GSTEMP.NAM_SAN_XUAT = NVL(BDDAU.NAM_SX, TSKHAC.NAM_SX),
        GSTEMP.NGAY_SU_DUNG = NVL(BDDAU.THOI_GIAN_SU_DUNG, TSKHAC.THOI_GIAN_SU_DUNG),
        GSTEMP.NGUYEN_GIA_BAN_DAU = ( NVL(BDDAU.NGUYEN_GIA_NS, 0) + NVL(BDDAU.NGUYEN_GIA_KHAC, 0) + NVL(BDDAU.NGUYEN_GIA_ODA, 0) + NVL(BDDAU.NGUYEN_GIA_VIEN_TRO, 0) ),
    ...
    COMMIT;
    RETURN 1;
END GS_UPDATE_TSCLN_45_TO_GSTEMP_DONG_BO;