...
{
    public partial class CCDC_GiamOrTang
    {
        private readonly BaoCaoTaiSanChiTietSearchModel _baoCaoTaiSanChiTietSearchModel;
        private readonly CauHinhBaoCaoModel _cauHinhBaoCaoModel;
        private readonly CauHinhBaoCaoChungModel _cauHinhBaoCaoChungModel;

        public CCDC_GiamOrTang(BaoCaoTaiSanChiTietSearchModel baoCaoTaiSanChiTietSearchModel,
        ...
        private void ReportHeader_BeforePrint(object sender, System.Drawing.Printing.PrintEventArgs e)
        {
            ...
            {
                CenterHeader.Text = "BẢNG TÌNH HÌNH GIẢM CÔNG CỤ, DỤNG CỤ";
                ngayANDbophan.Text = $"Kỳ báo cáo từ ngày: { _baoCaoTaiSanChiTietSearchModel.NgayBatDau.toDateVNString()} đến ngày {_baoCaoTaiSanChiTietSearchModel.NgayKetThuc.toDateVNString()} \r\n Lý do giảm: {_baoCaoTaiSanChiTietSearchModel.StringLyDoTangGiam}";
            }
            else if (_baoCaoTaiSanChiTietSearchModel.LyDoID == 1)
            {
                CenterHeader.Text = "BẢNG TÌNH HÌNH TĂNG CÔNG CỤ, DỤNG CỤ ";
                ngayANDbophan.Text = $"Kỳ báo cáo từ ngày: { _baoCaoTaiSanChiTietSearchModel.NgayBatDau.toDateVNString()} đến ngày {_baoCaoTaiSanChiTietSearchModel.NgayKetThuc.toDateVNString()} \r\n  Lý do tăng: {_baoCaoTaiSanChiTietSearchModel.StringLyDoTangGiam}";
            }
            else
            {
                CenterHeader.Text = "BẢNG TÌNH HÌNH TĂNG, GIẢM CÔNG CỤ, DỤNG CỤ  ";
                ngayANDbophan.Text = $"Kỳ báo cáo từ ngày: { _baoCaoTaiSanChiTietSearchModel.NgayBatDau.toDateVNString()} đến ngày {_baoCaoTaiSanChiTietSearchModel.NgayKetThuc.toDateVNString()} \r\n Lý do tăng/ giảm: {_baoCaoTaiSanChiTietSearchModel.lstNhanHienThi.Where(c => c.Key == ("List." + new enumLyDoGiam().GetType().FullName)).Select(c => c.Value).FirstOrDefault()}";
            }
            //ngayANDbophan.Text = "Kỳ báo cáo từ ngày: " + _baoCaoTaiSanChiTietSearchModel.NgayBatDau.toDateVNString() + " đến ngày " + _baoCaoTaiSanChiTietSearchModel.NgayKetThuc.toDateVNString() /*+ " \r\n "+ "Phòng ban, bộ phận sử dụng: " +_donvibophan*/;
        }
        ...
        {
            var row = GetCurrentRow() as TangGiamCongCu;
            if (row == null)
            {
