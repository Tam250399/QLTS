CREATE OR REPLACE FUNCTION GS_UPDATE_TSVKT_45_TO_GSTEMP_DONG_BO (
...
-- 92	  Qu?n lý nhà n??c	  2	              	      1	          1
-- 93	  H?SN-Không KD	      2	              1	      1	          2
-- 94	  H?SN-Kinh doanh	    2	              1	      1	          3
-- 95	  S? d?ng khác	      2	              0	      1	          6
-- 96	  H?SN-LDLK	          2	              	      1	          4
-- 97	  H?SN-Cho thuê	      2	              0	      1	          5
-- 186	Qu?n lý d? án	      2	              2	      1	          1
-- 187	S? d?ng khác	      2	              2	      1	          2
    SELECT
        *
    INTO GSTEMPDONGBO
    FROM
...
--
    ...
    FOR R_BD_VKT IN (
        ...
        BIENDONGJSONGUID := RAW_TO_GUID(LPAD(R_BD_VKT.BD_KHAC_ID, 32, '0'));
        IF R_BD_VKT.LOAI_BIEN_DONG = 4 THEN
            R_BD_VKT.MA_LY_DO := '047';
        END IF;
        BEGIN
            SELECT
                GSTG.*
            ...
            WHERE
                ROWNUM = 1
                AND GSTG.MA_OLD = R_BD_VKT.MA_LY_DO;
            ...
            WHERE
                MA_OLD = (
                    ...
                    FROM
                        DM_LOAITAISAN
                    WHERE
                        LOAI_TAI_SAN_ID = R_BD_VKT.LOAI_TAI_SAN_ID
                )
        ...
        IF R_BD_VKT.MA_LY_DO IN (
            ...
            BEGIN
                ...
                WHERE
                    ...
                    AND NGAY_BIEN_DONG < R_BD_VKT.NGAY_BIEN_DONG
                ORDER BY
                    NGAY_BIEN_DONG DESC
        ...
        "NGUYEN_GIA":'
                        || RTRIM(TO_CHAR((CASE
            WHEN R_BD_VKT.MA_LY_DO IN(
                '050', '051', '052'
            ) THEN
                ABS(NVL(R_BD_VKT.NGUYEN_GIA_NS, 0) + NVL(R_BD_VKT.NGUYEN_GIA_KHAC, 0) - NVL(BDKHACCUOI.NGUYEN_GIA_NS, 0) - NVL(BDKHACCUOI.NGUYEN_GIA_KHAC, 0))
            ELSE NVL(R_BD_VKT.NGUYEN_GIA_NS, 0) + NVL(R_BD_VKT.NGUYEN_GIA_KHAC, 0)
        END), 'FM9999999999999999999999990.99'), '.')
                        || ',
        "NGAY_BIEN_DONG":"'
                        || TO_CHAR(R_BD_VKT.NGAY_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "DON_VI_NHAN_DIEU_CHUYEN_MA":"'
                        || R_BD_VKT.MA_DONVI_NHAN_DIEU_CHUYEN
                        || '",
        "LOAI_BIEN_DONG_ID":'
                        || R_BD_VKT.LOAI_BIEN_DONG
                        || ',
        "LOAI_TAI_SAN_MA":"'
                        || GSMAPLTS.MA_NEW
                        || '",
        ...
        "HINH_THUC_XU_LY_ID":'
                        || (
            CASE
                WHEN NVL(R_BD_VKT.HINH_THUC_THANH_LY, R_BD_VKT.HINH_THUC_BAN) = '001' THEN
                    2
                WHEN NVL(R_BD_VKT.HINH_THUC_THANH_LY, R_BD_VKT.HINH_THUC_BAN) = '002' THEN
                    3
                WHEN NVL(R_BD_VKT.HINH_THUC_THANH_LY, R_BD_VKT.HINH_THUC_BAN) = '003' THEN
                    1
            END
        )
                        || ',
        "DON_VI_MA":"'
                        ...
                        || '",
        "NGAY_SU_DUNG":"'
                        || TO_CHAR(R_BD_VKT.THOI_GIAN_SU_DUNG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "THONG_SO_KY_THUAT":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_VKT.THONG_SO_KT)
                        || '",
        "GHI_CHU":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_VKT.MO_TA_CHUNG)
                        || '",
        "NGAY_DUYET":"'
                        || TO_CHAR(R_BD_VKT.NGAY_DUYET_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "TRANG_THAI":'
                        || NVL(R_BD_VKT.DUYET_BIEN_DONG, 1)
                        || ',
        "GIA_TRI_CON_LAI":'
                        || RTRIM(TO_CHAR(R_BD_VKT.GIA_TRI_CON_LAI, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "KH_GIA_TINH_KHAU_HAO":'
                        || RTRIM(TO_CHAR(NVL(R_BD_VKT.KH_NGUYEN_GIA_NS, 0) + NVL(R_BD_VKT.KH_NGUYEN_GIA_KHAC, 0) + NVL(R_BD_VKT.KH_NGUYEN_GIA_ODA, 0) + NVL(R_BD_VKT.KH_NGUYEN_GIA_VIEN_TRO, 0), 'FM9999999999999999999999990.99'
                        ), '.')
                        || ',
        "KH_GIA_TRI_CON_LAI":'
                        || RTRIM(TO_CHAR(R_BD_VKT.KH_GIA_TRI_CON_LAI, 'FM9999999999999999999999990.99'), '.')
                        || ',
        "KH_THANG_CON_LAI":'
                        || R_BD_VKT.SO_THANG_CON_SU_DUNG
                        || ',
        "QUYET_DINH_SO":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_VKT.SO_QD)
                        || '",
        "QUYET_DINH_NGAY":"'
                        || TO_CHAR(R_BD_VKT.NGAY_BIEN_DONG, 'yyyy-MM-dd HH:MI:SS')
                        || '",
        "PHI_THU":'
                        || RTRIM(TO_CHAR(R_BD_VKT.SO_TIEN_THU_DUOC, 'FM9999999999999999999999990.99'), '.')
                        || ',   
        "HS_KHAC":"'
                        || GS_STRING_TO_STRINGJSON(R_BD_VKT.SO_QD)
                        || '"},';
        ...
        NGUONVONJSON := NGUONVONJSON
                        ...
                        || R_BD_VKT.NGUYEN_GIA_NS
                        || ', "BIEN_DONG_GUID":"'
                        || BIENDONGJSONGUID
                        || '"}, 
        {"NGUON_VON_ID":3, "GIA_TRI":'
                        || R_BD_VKT.NGUYEN_GIA_KHAC
                        || ', "BIEN_DONG_GUID":"'
                        || BIENDONGJSONGUID
                        || '"},';
        ...
        IF GSTEMPDONGBO.MA_DU_AN IS NULL THEN
          ...
          "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_VKT.HIEN_TRANG_SU_DUNG IN (
                        '0',
                        '1'
                    ) OR R_BD_VKT.HIEN_TRANG_SU_DUNG IS NULL THEN
                        'true'
                    ELSE 'false'
                END
            )
          ...
          "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_VKT.HIEN_TRANG_SU_DUNG = '3' THEN
                        'true'
                    ELSE 'false'
                END
            )
          ...
          "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_VKT.HIEN_TRANG_SU_DUNG = '2' THEN
                        'true'
                    ELSE 'false'
                END
            )
          ...
          "GIA_TRI_CHECKBOX":'
                             || (
                CASE
                    WHEN R_BD_VKT.HIEN_TRANG_SU_DUNG = '4' THEN
                        'true'
                    ELSE 'false'
                END
            )
    ...
    COMMIT;
    RETURN 0;
END GS_UPDATE_TSVKT_45_TO_GSTEMP_DONG_BO;