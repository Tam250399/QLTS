...
namespace GS.Web.Framework.Validators
{
    ...
    public abstract class BaseGSValidator<TModel> : AbstractValidator<TModel> where TModel : class
    {
        #region Ctor

        protected BaseGSValidator()
        {
            PostInitialize();
        }
        ...
        protected virtual void SetDecimalMaxValue<TEntity>(IDbContext dbContext) where TEntity : BaseEntity
        {
            ...
            foreach (var expression in maxValueExpressions)
            {
                RuleFor(expression.Expression).IsDecimal(expression.MaxValue)
                    .WithMessage(string.Format(_nhanHienThiService.GetGiaTri("GS.Web.Framework.Validators.MaxDecimal"), expression.MaxValue - 1));
            }
        }
