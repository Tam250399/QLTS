...
namespace GS.Web.Framework.TagHelpers.Shared
{
    ...
    public class GSDatePickerTagHelper : TagHelper
    {
        ...
        public GSDatePickerTagHelper(IHtmlGenerator generator, IHtmlHelper htmlHelper)
        {
            Generator = generator;
        ...
        public override void Process(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var locService = EngineContext.Current.Resolve<INhanHienThiService>();

            days.AppendFormat("<option value='{0}'>{1}</option>", "0", locService.GetGiaTri("Common.Day"));
            for (var i = 1; i <= 31; i++)
                days.AppendFormat("<option value='{0}'{1}>{0}</option>", i,
                    (SelectedDay.HasValue && SelectedDay.Value == i) ? " selected=\"selected\"" : null);

            months.AppendFormat("<option value='{0}'>{1}</option>", "0", locService.GetGiaTri("Common.Month"));
            for (var i = 1; i <= 12; i++)
            {
            ...
            years.AppendFormat("<option value='{0}'>{1}</option>", "0", locService.GetGiaTri("Common.Year"));
