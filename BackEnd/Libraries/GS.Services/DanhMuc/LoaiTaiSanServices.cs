...
{
	public partial class LoaiTaiSanService : ILoaiTaiSanService
	{
		#region Fields
		...
		#endregion
		#region Ctor
		public LoaiTaiSanService(CauHinhChung cauhinhChung,
		...
		{
			var query = _itemRepository.Table.Where(c => c.LOAI_HINH_TAI_SAN_ID < 12);
		...
		public virtual IPagedList<LoaiTaiSan> SearchLoaiTaiSanToanDan(int pageIndex = 0, int pageSize = int.MaxValue, string Keysearch = null, decimal? ParentId = 0, decimal? ChedoId = 0, decimal? loaiHinhTaiSanId = 0, decimal? id = 0, bool isHasNoChildrend = false)
		{
			var query = _itemRepository.Table.Where(c => c.LOAI_HINH_TAI_SAN_ID > 11);
			if (!String.IsNullOrEmpty(Keysearch))
			{
				Keysearch = Keysearch.ToUpper();
				query = query.Where(c => c.TEN.ToUpper().Contains(Keysearch) || c.MA.ToUpper().Contains(Keysearch));
		...
		public IList<LoaiTaiSan> GetListLoaiTaiSanDatNhaTreeNodeByRoot(decimal? cheDoHaoMonId = 0, decimal? LoaiHinhTaiSanId = 0, int [] ListLoaiHinhTaiSanId = null, decimal? ParentLoaiTaiSanId = null, List<decimal> ListIgnoreLTS = null)
		{
			var query = _itemRepository.Table;
			if (cheDoHaoMonId > 0)
			{
			...
			if (ListLoaiHinhTaiSanId != null && ListLoaiHinhTaiSanId.Count() > 0)
			{
				query = query.Where(c => ListLoaiHinhTaiSanId.Contains((int)c.LOAI_HINH_TAI_SAN_ID));
			}
			if (ListIgnoreLTS != null && ListIgnoreLTS.Count() > 0)
			{
				query = query.Where(c => !ListIgnoreLTS.Contains((decimal)c.LOAI_HINH_TAI_SAN_ID));
