...
{
   public  interface ITaiSanSvcFactory
    {
        #region TaiSan
        ResultTaiSan GetAllTaiSans(int LoaiHinhTaiSanId=0, int pageSize = int.MaxValue, int pageIndex = 0, int DonViId=0);
        MessageReturn UpdateTaiSan(TaiSanDBModel model, NguoiDung currentUser);
        MessageReturn UpdateListTaiSan(List<TaiSanDBModel> ListModel, NguoiDung currentUser, decimal nguontaisan);
        MessageReturn DeleteTaiSan(string MaTaiSan);
        MessageReturn UpdateBienDong(List<BienDongDBModel> ListBienDong, NguoiDung currentUser, decimal nguontaisan);
